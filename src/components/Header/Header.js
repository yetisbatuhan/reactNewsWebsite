...
const Header = () => {
    ...
    return (
        <div className=' flex justify-between container mx-auto py-3 '>
            <div className='logo  '>
             <h1 className='sm:text-[20px] block  xl:text-[50px]  font-serif '>2DAY <span className='border bg-red-500 text-white rounded-b-lg px-2 font-extrabold '>News</span></h1>
            ...
            <img width="20" height="20" className='md:hidden' src="https://img.icons8.com/ios/50/000000/menu--v7.png" alt="menu--v7"/>
               {menuList.map((category, index) => (
                  <a className='sm:px-1 md:px-2 pt-3 font-bold text-[20px] sm:hidden md:flex border-gray-50' href={category} key={index}>{category}</a>
              ))}
